...
class GenericTreeItem(object):
    ...
    def GetStatusImage(self):
        """
        Returns the status image associated to this item, i.e. the image on the
        leftmost part of the item area of :class:`CustomTreeCtrl`.
    ...
    def SetStatusImage(self, image):
        """
        Sets the item status image, i.e. the image associated to the item on the leftmost
        part of the :class:`CustomTreeCtrl` client area.

        :param integer `image`: an index within the left image list specifying the image to
         use for the item in the leftmost part of the item area.
        """

        self._statusimage = image
...
class CustomTreeCtrl(wx.ScrolledWindow):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize,
        ...
        self._imageListLeft = self._imageListStatus = None
        self._ownsImageListNormal = self._ownsImageListButtons = self._ownsImageListState = False
        self._ownsImageListLeft = self._ownsImageListStatus = False
...
# -----------------------------------------------------------------------------
    ...
    def GetItemSize(self, item):
        ...
        image = item.GetCurrentImage()
        statusimage = item.GetStatusImage()
        ...
        if statusimage != _NO_IMAGE:
            if self._imageListStatus:
                s_image_w, dummy = self._imageListStatus.GetSize(image)
                s_image_w += 4

        maxsize = w - (wcheck + s_image_w + image_w + item.GetX()) + xa
        return maxsize
    ...
    def GetItemStatusImage(self, item):
        """
        Returns the item status image, i.e. the image associated to the item status to left of label
        of the :class:`CustomTreeCtrl` client area.

        :param `item`: an instance of :class:`GenericTreeItem`.

        :return: An integer index that can be used to retrieve the item status image inside
         a :class:`ImageList`.
        """

        return item.GetStatusImage()
    ...
    def SetItemStatusImage(self, item, image):
        """
        Sets the item status image, i.e. the image associated to the item on the left of the label
        of the :class:`CustomTreeCtrl` client area.

        :param `item`: an instance of :class:`GenericTreeItem`;
        :param integer `image`: an index within the status image list specifying the image to
         use for the item on the left of the label of the client area.
        """

        item.SetStatusImage(image)
...
# -----------------------------------------------------------------------------
    ...
    def DoInsertItem(self, parentId, previous, text, ct_type=0, wnd=None, image=-1, selImage=-1, data=None,
        ...
        if not parent:
            ...
            return self.AddRoot(text, ct_type, wnd, image, selImage, data, statusImage)
        
        self._dirty = True     # do this first so stuff below doesn't cause flicker

        item = GenericTreeItem(parent, text, ct_type, wnd, image, selImage, data, separator, statusImage)
    ...
    def AddRoot(self, text, ct_type=0, wnd=None, image=-1, selImage=-1, data=None, statusImage=-1):
        """
        ...
        :param  integer `selImage`: an index within the normal image list specifying the image to
         ...
         same image is used for both selected and unselected items;
        :param object `data`: associate the given Python object `data` with the item.
        :param object `statusImage`: an index within the status image list.
        ...
        self._anchor = GenericTreeItem(None, text, ct_type, wnd, image, selImage, data, False, statusImage)
    ...
    def PrependItem(self, parent, text, ct_type=0, wnd=None, image=-1, selImage=-1, data=None, separator=False,
        ...
        return self.DoInsertItem(parent, 0, text, ct_type, wnd, image, selImage, data, separator, statusImage)
    ...
    def InsertItemByItem(self, parentId, idPrevious, text, ct_type=0, wnd=None, image=-1, selImage=-1, data=None,
        ...
        return self.DoInsertItem(parentId, index+1, text, ct_type, wnd, image, selImage, data, separator, statusImage)
    ...
    def AppendItem(self, parentId, text, ct_type=0, wnd=None, image=-1, selImage=-1, data=None, statusImage=-1):
        """
        ...
        return self.DoInsertItem(parent, len(parent.GetChildren()), text, ct_type, wnd, image, selImage, data, False,
                                 statusImage)
    ...
    def GetStatusImageList(self):
        """
        Returns the image list for :class:`CustomTreeCtrl` filled with images to be used on
        the status part of the client area. Any item can have a status image associated
        with it.

        :return: An instance of :class:`ImageList`.
        """

        return self._imageListStatus
    ...
    def CalculateLineHeight(self):
        ...
        if self._imageListLeft:
            ...
            for i in range(n):
                ...
                width, height = self._imageListStatus.GetSize(i)
    ...
    def SetStatusImageList(self, imageList):
        """
        Sets the image list for :class:`CustomTreeCtrl` filled with images to be used on
        the status part of the client area. Any item can have a status image associated
        with it.
        ...
        self._imageListStatus = imageList
        self._ownsImageListStatus = False
        self._dirty = True
    ...
    def AssignStatusImageList(self, imageList):
        """
        Assigns the image list for :class:`CustomTreeCtrl` filled with images to be used on
        the status part of the client area. Any item can have a status image associated
        with it.
        ...
        self.SetStatusImageList(imageList)
        self._ownsImageListStatus = True
...
# -----------------------------------------------------------------------------
    ...
    def PaintItem(self, item, dc, level, align):
        ...
        if statusimage != _NO_IMAGE:
            s_image_w, s_image_h = self._imageListStatus.GetSize(statusimage)
            s_image_w += 2
        ...
        if self.HasAGWFlag(TR_FULL_ROW_HIGHLIGHT):
            ...
            if item.IsSelected():
               ...
               if drawItemBackground:
                   minusicon = wcheck + s_image_w + image_w - 2
            ...
            if item.IsSelected():
                ...
                if separator:
                    ...
                    item_width = item.GetWidth() - s_image_w - image_w - wcheck + 2 - wndx
                    
                itemrect = wx.Rect(item.GetX() + wcheck + s_image_w + image_w - 2,
                                   item.GetY()+offset,
                                   item_width,
                                   total_h-offset)
        ...
        if statusimage != _NO_IMAGE:
            ...
            dc.SetClippingRegion(item.GetX(), item.GetY(), wcheck+s_image_w-2, total_h)
            if item.IsEnabled():
                imglist = self._imageListStatus
            else:
                imglist = self._grayedImageListStatus

            imglist.Draw(statusimage, dc,
                         item.GetX() + wcheck,
        ...
        if image != _NO_IMAGE:
            ...
            dc.SetClippingRegion(item.GetX(), item.GetY(), wcheck+s_image_w+image_w-2, total_h)
            if item.IsEnabled():
        ...
        textrect = wx.Rect(wcheck + s_image_w + image_w + item.GetX(), item.GetY() + extraH, text_w, text_h)
        ...
        wnd = item.GetWindow()
        if wnd:
            wndx = wcheck + s_image_w + image_w + item.GetX() + text_w + 4
            xa, ya = self.CalcScrolledPosition((0, item.GetY()))
            wndx += xa
    ...
    def CalculateSize(self, item, dc, level=-1, align=0):
        ...
        if statusimage != _NO_IMAGE:
            ...
            if self._imageListStatus:
                ...
                s_image_w, s_image_h = self._imageListStatus.GetSize(image)
                s_image_w += 4

        total_h = ((s_image_h > text_h) and [s_image_h] or [text_h])[0]
        ...
        wnd = item.GetWindow()
        if not wnd:
            totalWidth = s_image_w+image_w+text_w+wcheck+2
            totalHeight = total_h
        else:
            totalWidth = item.GetWindowSize()[0]+s_image_w+image_w+text_w+wcheck+2
            totalHeight = max(total_h, item.GetWindowSize()[1])

        if level >= 0 and wnd:
            if align == 0:
                if level in self.absoluteWindows:
                    self.absoluteWindows[level] = max(self.absoluteWindows[level], s_image_w+image_w+text_w+wcheck+2)
                else:
                    self.absoluteWindows[level] = s_image_w+image_w+text_w+wcheck+2
            elif align == 1:
                self.absoluteWindows[level] = max(self.absoluteWindows[level], s_image_w+image_w+text_w+wcheck+2)
